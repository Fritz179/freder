...
use super::{One, Transform2D, Transformable, Zero};
    ...
    ($trait:ident, $func:ident, $op:tt, $trait_assign:ident, $func_assign:ident, $op_assign:tt) => {
            ...
            Lhs: std::ops::$trait_assign<Rhs>,
        {
            fn $func_assign(&mut self, rhs: Vec2<Rhs>) {
                // self.0[0] $op_assign rhs.x;
                // self.0[1] $op_assign rhs.y;
...
impl<T: Copy, S: Copy> Transformable<T, S> for Vec2<T>
where
    Vec2<T>: std::ops::AddAssign<Vec2<T>>,
    Vec2<T>: std::ops::MulAssign<S>,
{
